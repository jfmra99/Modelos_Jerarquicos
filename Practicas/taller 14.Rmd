...
```{r}
## creamos un grid de prediccion
df_pred <- data.frame(time = seq(0,18, length.out=500),
               treatment = rep(c("terbinafine", "itraconazole", "terbinafine", "terbinafine"), each=500),
               patientID = factor(as.character(rep(c(1, 3, 21, 383), each=500)))
                               )
## estimamos la predicción del modelo
df_pred$estimate <- predict(mod8, newdata = df_pred, type = "response")
## graficamos clarificando que los datos son los de la predicción, importante tener los mismos nombres en las variables, pero esto debería estar garantizado si te corrio la prediccion
